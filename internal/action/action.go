...
func (a *Action) HandleMatch(scan string, sourceName string, version string, args map[string]string) (bool, string, error) {
	// check with source for the version
	s, err := source.Get(sourceName)
	if err != nil {
		return false, "", err
	}
	curVer, err := s.Get(args)
	if err != nil {
		return false, "", err
	}
